...
#define MEMOSIZE 1024 // Size of Ram disk
# define KERNSIZE (512)
int d=0; //Variable for Major Number
int  sectsize = 512;
...
static int my_opn(struct block_device *x, fmode_t mode)
{
	int ret=0;
...
static void my_rel(struct gendisk *disk, fmode_t mode)
{
...
{
	(device.data) = vmalloc(MEMOSIZE*sectsize);
...
static int rb_trans(struct request *req)
{
...
static void dev_req(struct request_queue *q)
{
	struct request *req;
	int error;
						      ...
						      *for data transfer*/
	{
		...
		error=rb_trans(req);// transfer the request for operation
...
{
	mydisk_init();
	d = register_blkdev(d, "dof");// major no. allocation
	printk(KERN_ALERT "Major Number is : %d",d);
	spin_lock_init(&device.lock); // lock for queue
	device.queue = blk_put_queue( dev_req, &device.lock);

	device.gd = alloc_disk(8); // gendisk allocation

	(device.gd)->major=d; // major no to gendisk
	...
	set_capacity(device.gd, MEMOSIZE*(KERNSIZE/sectsize));
	add_disk(device.gd);
